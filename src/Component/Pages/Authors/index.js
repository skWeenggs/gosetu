...
const Authors = () => {
    ...
    console.log(data);
        ...
        useEffect(() => {
          ...
          if (matchingDomain) {
              setDatabaseId(matchingDomain.properties.AuthorPageId.rich_text[0]?.plain_text);
   ...
   const Datafilter=(fil)=>{
    console.log("dsd",fil);
    navigate(`/authors/${fil?.properties?.Slug?.rich_text[0]?.plain_text}`, { state: { myData: fil, domain: domain } })
   }
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div >
          {data && data.users?.results.map((list,i)=>{
            return(
              <>
              <div  onClick={() => Datafilter(list)} className='max-w-2xl mx-auto px-4 sm:px-6 py-4 items-center lg:px-8 flex justify-between w-full text-gray-500  hover:text-gray-900 cursor-pointer'>
               <p className='text-lg text-center'> {list?.icon?.emoji} </p>
               {list?.icon?.external?.url ?(
                 <><img src={list?.icon?.external?.url} alt='' className='h-10 w-10 rounded-full '/></> 
                 ):list?.icon?.file?.url ?( <img src={list?.icon.file.url} alt="" className='h-5 w-5' />):''}
                <p className='truncate whitespace-normal text-lg font-semibold text-gray-900 sm:text-xl sm:ml-5 lg:text-2xl'>
